...
def pairwise_distances(X, Y=None, metric='euclidean'):
    ...
    match(metric.lower()):
        case 'euclidean':
            ...
            x_sq = np.sum(arr_X * arr_X, axis=1, keepdims=True)    # (n,1)
            y_sq = np.sum(arr_Y * arr_Y, axis=1, keepdims=True).T  # (1,m)
            cross = arr_X @ arr_Y.T                                # (n,m)
            sq_dist = x_sq + y_sq - 2.0 * cross
            # float error can push tiny diagonal values slightly negative
            sq_dist = np.maximum(sq_dist, 0.0)
            return np.sqrt(sq_dist)
        case 'manhattan':
            # broadcasting trick: (n,1,d) - (1,m,d) -> (n,m,d), sum on d
...
def topk_indices(values, k, largest=True):
    ...
    n = arr.shape[-1]
    if k > n:
        k = n  # clamp so we dont ask for more elements than exist
    # argpartition is O(n) and gives us the k extreme indices but unsorted
    # then we do a tiny argsort over just those k to get them in order
    if largest:
        part_idx = np.argpartition(-arr, k - 1, axis=-1)[..., :k]
    else:
        part_idx = np.argpartition(arr, k - 1, axis=-1)[..., :k]
    # now order the small k-slice properly
...
def iter_batches(n_samples, batch_size, shuffle=False, seed=None):
    ...
    if shuffle:
        # default_rng is the modern numpy RNG, seed lets you reproduce
        rng = np.random.default_rng(seed)
        rng.shuffle(indices)
    # yield slices one at a time -> caller does X[idx] themselves
    # last batch can be smaller than batch_size if n doesnt divide evenly
